  ...
  <div class="el-slider wx-slider"
    ...
    <div class="el-slider__runway"
      ...
      <div
        ...
        :key="key"
        :style="vertical ? { 'bottom': (100 - item) + '%' } : { 'left': (100 - item) + '%' }"
        v-if="showStops"
        :class="{'active': diffStops(item)}">
        <span class="el-slider__mark">{{min + item * (max - min) / 100}}</span>
      </div>
    </div>
  </div>
</template>
  ...
  computed: {
    ...
    barSize() {
      return this.range
        ...
        : this.reverseStops
          ? `${ 100 * (this.max - this.firstValue) / (this.max - this.min) }%`
...
<style lang="scss">
  ...
  .el-tooltip__popper {
    ...
    .popper__arrow {
      border-color: #42b983 transparent transparent transparent !important;
      &:after {
        border-color: #42b983 transparent transparent transparent !important;
      }
    }
